...
def validate_file(ctx, param, value):
    ...
    if not path.exists():
        raise click.BadParameter(
            f"No file present for model '{path.stem}', missing file: '{str(path.resolve())}'"
        )
...
def get_summed_val_size():
    ...
    if cached_val_file.exists():
        ...
        config = util.get_config()
        val_size = sum_sizes_from_file(
            util.get_sha_path_from_result(config["result_test_csv"])
        )
...
@click.option(
    "--cache",
    "cache_dir",
    help="Path to the cache dir, where the model's DF could be stored",
)
@click.option(
    "--source",
    "source_dir",
    help="Path to the directory containing the calculated source metrics (runtime, size, fmes, etc.)",
...
@click.option(
    ...
    help="The model to calculate the df for",
)
...
def get(cache_dir, source_dir, model_name, metric, value):
    vals, normalized_vals, cum_vals = get_data(
        cache_dir, source_dir, model_name, metric
    )

...
def get_ranks_by_weeks(runs_data, n_run, arch, model_name, metric):
    ...
    return res, current_model_rank_sum / week_count, current_model_rank_sum / all_models_rank_sum
    
def get_aggregated_ranks_by_weeks(runs_data, n_run, arch, model_name):
    prec_ranks, avg_prec, norm_prec = get_ranks_by_weeks(runs_data, n_run, arch, model_name, 'precision')
    recall_ranks, avg_recall, norm_recall = get_ranks_by_weeks(runs_data, n_run, arch, model_name, 'recall')
    memory_ranks, avg_memory, norm_memory = get_ranks_by_weeks(runs_data, n_run, arch, model_name, 'memory')
    time_ranks, avg_time, norm_time = get_ranks_by_weeks(runs_data, n_run, arch, model_name, 'time')
    
    res = {
        'precision' : {
            'raw' : prec_ranks,
            'average' : avg_prec,
            'normalized' : norm_prec
        },
        'recall' : {
            'raw' : recall_ranks,
            'average' : avg_recall,
    ...
    for n_run in util.get_runs_range():
        ...
        for model_name in util.get_model_names():
            ...
            for arch in util.get_arch_types():
                ...
                model_ranks[n_run][model_name][arch]['ranks'] = get_aggregated_ranks_by_weeks(runs_data, n_run, arch, model_name)
                model_ranks[n_run][model_name][arch]['utility_score'] = get_utility_score(model_ranks[n_run][model_name][arch]['ranks'], 
                                                                                                   usecase, 
                                                                                                   'normalized')
...
def dump_csvs(ranks_by_model):
    for n_run in util.get_runs_range():
        for arch in util.get_arch_types():
            ...
            sorted_models = sorted(ranks_by_model[n_run].items(), key = lambda item: item[1][arch]['utility_score'], reverse=True)
    ...
    for n_run in util.get_runs_range():
        ...
        for n_week in util.get_weeks_range():
            ...
            for arch in util.get_arch_types():
                ...
                curr_path = util.get_eval_results_path() / str(n_run) / f"{task_id}.json"
                ...
                runs_data[n_run][n_week][arch]['scores']['precision'] = sort_dict(prec_scores)
                runs_data[n_run][n_week][arch]['scores']['recall'] = sort_dict(recall_scores)
                runs_data[n_run][n_week][arch]['scores']['memory'] = sort_dict(memory_scores, True)
                runs_data[n_run][n_week][arch]['scores']['time'] = sort_dict(time_scores, True)
                
                runs_data[n_run][n_week][arch]['ranks'] = {}
                runs_data[n_run][n_week][arch]['ranks']['precision'] = get_ranks(runs_data[n_run][n_week][arch]['scores']['precision'])
                runs_data[n_run][n_week][arch]['ranks']['recall'] = get_ranks(runs_data[n_run][n_week][arch]['scores']['recall'])
                runs_data[n_run][n_week][arch]['ranks']['memory'] = get_ranks(runs_data[n_run][n_week][arch]['scores']['memory'])
                runs_data[n_run][n_week][arch]['ranks']['time'] = get_ranks(runs_data[n_run][n_week][arch]['scores']['time'])

...
def get_weekly_files_to_merge(
    benign_root_path: Path,
    malware_root_path: Path,
    n_run: int,
    n_weeks: List[int],
    architecture: str,
    suffix: str,
    ...
    for n_week in n_weeks:
        ...
        files_to_merge += [
            benign_root_path / bening_file_path,
            malware_root_path / malware_file_path,
        ]
...
def init_result_path(architecture, n_run, n_week, suffix):
    ...
    weekly_prefix = config["weekly_prefix"].replace("<architecture>", architecture)
    result_path = (
        util.get_eval_input_path()
        / architecture
        / get_file_part(weekly_prefix, n_run, n_week, suffix, "merged")
    )
...
def init_train_files(architecture: str):
    ...
    bening_filename = (
        config["boosting_filename"]
        .replace("<architecture>", architecture)
        .replace("<label>", "b")
    )
    benign_boosting_path = (
        Path(config["weekly_root_dir"])
        / architecture
        / "benign"
        / f"{bening_filename}.csv"
    )
    malware_filename = (
        config["boosting_filename"]
        .replace("<architecture>", architecture)
    ...
    malware_boosting_path = (
        Path(config["weekly_root_dir"])
        / architecture
        / "malware"
        / f"{malware_filename}.csv"
    )
    ...
    for n_run in range(config["start_run"], config["end_run"] + 1):
        for n_week in range(config["start_week"], config["end_week"] + 1):
            ...
            if weeks_to_merge:
                files_to_merge += get_weekly_files_to_merge(
                    benign_root_path,
                    malware_root_path,
                    n_run,
                    weeks_to_merge,
            ...
            create_dataset.main(
                [*files_to_merge, str(result_path.resolve())], standalone_mode=False
            )

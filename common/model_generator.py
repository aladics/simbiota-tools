...
class TaskFactory:
    ...
    def get(self, model_name, sargs):
        if model_name != "cdnnc" and model_name != "sdnnc":
            ...
            learn_task = self.DNNCLearnTask(
                self.shared_params, str((sandbox_path / model_name).resolve())
            )
...
def generate_param_configs(parsed_params, n):
    ...
    for par_name, par_config in parsed_params.items():
        if par_config["type"] == "uniform_int":
            generated_params[par_name] = np.random.random_integers(
                int(par_config["from"]), int(par_config["to"]), n
            ).tolist()
        if par_config["type"] == "uniform_float":
            generated_params[par_name] = np.random.uniform(
                float(par_config["from"]), float(par_config["to"]), n
            ).tolist()
...
def generate_configs(param_configs):
    ...
    for param_config in param_configs:
        configs += [
            dict(zip(param_config.keys(), param))
            for param in zip(*param_config.values())
        ]
...
def parse_params(raw_params):
    ...
    for key, val in dependent_pars.items():
        ...
        for dependency_name, dependency_details in val["depends_on"].items():
            ...
            param_configs[0][dependency_name]["values"].remove(
                dependency_details["values"]
            )
...
def generate_model(
    model_name, raw_params, shared_params, save_path, train_file, test_file, n
):
...
def run(search_params, save_path, train_file, test_file, n, log_file, standardize):
    ...
    for key, val in params.items():
        start = time.time()
        times = generate_model(
            key, val, shared_params, save_path, train_file, test_file, n
        )
        time_stats[key] = time.time() - start

        logger.info(
            f" {key}(rand_n = {n}) done. Elapsed: {round(time_stats[key], 3)} secs ({ceil(time_stats[key] / 60)} mins).\nExecution times per model:\n{str(times)}"
        )
